...
func FlagAuthorization(constructor *e2e.Constructor) func() {
	return func() {
		BeforeEach(func() {
			constructor.RepoAuthorization = uuid.New().String()

			constructor.Flags = map[string]string{
				testutil.StartFlagProjectPath:   testutil.GoExample1,
				testutil.StartFlagAuthorization: constructor.RepoAuthorization,
			}
		})
		...
		It("Shows the repository property", func() {
			Expect(constructor.Session.Wait(2 * time.Minute).Out.Contents()).To(ContainSubstring("repository_authorization"))
		})

		It("The authorization token is set", func() {
			Eventually(constructor.Session).Should(gbytes.Say(fmt.Sprintf("%s", constructor.RepoAuthorization)))
		})
